...
print('웹캠을 사용하지 않으시려면 미리 사진을 업로드 해 주세요.')
print('웹캠으로 사진을 찍으시는 경우 눈이 잘 보이게 찍어주시고 사진으로 관상을 보시는 경우 눈이 잘 보이는 사진으로 업로드 해 주세요')
print('a를 누르면 당신의 나이의 기댓값을 알 수 있으며 관상을 알아보기 위한 사진이 자동으로 촬영되어 저장됩니다.')
print('q를 누르면 웹캠이 종료되고 관상 결과를 알 수 있습니다.')
print('눈을 인식하지 못했다는 메세지가 뜬다면 웹캠을 다시 실행시키거나 눈이 잘보이는 사진으로 업로드 해 주세요.')
...
# "Deep Neural Networks”를 나타내는 OpenCV의 dnn 패키지를 사용
# dnn 패키지에서 OpenCV는 신경망을 채우는 데 사용할 수 있는 Net이라는 클래스를 제공
MODEL_MEAN_VALUES = (78.4263377603, 87.7689143744, 114.895847746)
age_list = ['(14, 16)', '(17, 19)', '(20, 22)', '(23, 25)', '(26, 28)', '(29, 31)', '(32, 35)', ('36, 40'), ('41, 50'), ('51, 60')]
...
while True:       
    ...
    ret, image = cap.read()
    face_cascade = cv2.CascadeClassifier('haarcascade_frontalface_alt10.xml')
    ...
    for (x, y, w, h )in faces:
        ...
        my_text = "%s %s" % ('expect age : ', int(cntage/cnt))
        
        if cv2.waitKey(1) & 0xFF == ord('a'):
            cv2.imwrite('face.jpg', image)
            cv2.putText(image, my_text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            print('예상 나이 : ', int(cntage/cnt))
            cnt = 0
            cntage = 0
        else :
...
cascadefile_right = "./haarcascade_righteye_2splits.xml" # 오른쪽 눈을 따로 인식
...
if len(facelist) >= 1: 
    for face in facelist: 
        ...
        cropped = imgray[y:y+h, x:x+w]  # 눈에 해당하는 부분을 추출함q
    result_filename = ["c:/Face Reader/left/left/lefteye.jpg"]
...
cascade_r = cv2.CascadeClassifier(cascadefile_right)
facelist_r = cascade_r.detectMultiScale(imgray, scaleFactor=2.08, minNeighbors=1) 

cropped=[]
if len(facelist_r) >= 1: 
    for face in facelist_r: 
        x, y, w, h = face 
        cv2.rectangle(imgray, (x, y), (x+w, y+h), (255, 0, 0), 2)   # 눈에 해당하는 부위를 네모로 표시
        cropped = imgray[y:y+h, x:x+w]   # 눈에 해당하는 부분을 추출함
    result_filename = ["c:/Face Reader/right/right/righteye.jpg"]   
    result_filename = ''.join(result_filename)
    cv2.imwrite(result_filename,cropped)  # 추출한 눈 부위를 저장
if not np.any(cropped):    # 눈을 인식하지 못했을 때
...
batch_size=16
testset = torchvision.datasets.ImageFolder(root='C:/Face Reader/left',transform=transform)
test_loader = torch.utils.data.DataLoader(dataset=testset, 
                                          batch_size = batch_size,
                                          shuffle=True)  # 추출했던 왼쪽눈을 testset으로 설정

testset1 = torchvision.datasets.ImageFolder(root='C:/Face Reader/right',transform=transform)
